...
export class BaseModel<T> {
    ...
    async create(data: Partial<T>): Promise<T> {
        const columns = Object.keys(data).filter(k => k !== this.idColumn).map(c => this.sanitizeIdentifier(c));
        const values = Object.keys(data).filter(k => k !== this.idColumn).map(k => data[k as keyof T]);
